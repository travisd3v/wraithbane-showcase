...
Videojuego inmersivo de plataformas, accion y combate contra jefes en 2D con tematica de fantasia oscura, desarrollado integramente desde cero sin motores comerciales (como Unity o Unreal) ni plantillas prefabricadas. El proyecto demuestra como estructurar una arquitectura desacoplada uniendo un motor grafico procedural de alto rendimiento en el navegador con integraciones en tiempo real para transmisiones en vivo.
...
## 1. Topologia de la Arquitectura
...
### Puente de Transmision (tiktok-bridge)
...
### Servidor de Control y Telemetria (TRAVIS)
...
*   **Empaquetador de Recursos:** Vite 8.1.3 para resolucion rapida de modulos, optimizacion de assets en compilacion de produccion y hot module replacement en desarrollo.
*   **Entorno de Pruebas:** Vitest 4.1.10 para la ejecucion de pruebas unitarias sobre los sistemas de fisica, calculo de dano y persistencia.
*   **Gestion de Dependencias:** pnpm 8.15.9 (arquitectura monorrepo con enlaces simbolicos de alta eficiencia).

### Servidores Backend
*   **Entorno de Ejecucion:** Node.js 20 o superior en todas las capas del servidor.
...
*   **Conexion Externas:** Librerias de conexion reversa para feed de streaming (tiktok-live-connector) y SDK oficial de Supabase para integraciones con bases de datos PostgreSQL en la nube.

### Despliegue e Infraestructura (Produccion)
*   **Alojamiento:** VPS dedicado Linux (Debian/Ubuntu).
*   **Aislamiento:** Contenedores Docker coordinados con Docker Compose bajo una red virtualizada privada y segura (wraithbane-net).
...
## 3. Arquitectura del Motor Grafico y Fisicas

El motor de Wraithbane fue programado sin frameworks de videojuegos. Sus modulos clave incluyen:

### Frame Loop y Temporizacion
...
*   El mapa se divide proceduralmente en zonas de progreso del 33% y 66% de su ancho.
...
## 4. Arquitectura del Sistema de Jefes (El Rey Espectro)
...
*   **Fase 1 (Acecho - 100% a 80% HP):** Patron defensivo con proyectiles parryables de rango largo (clase AlmaErrante).
*   **Fase 2 (Colera - 80% a 55% HP):** Patrones de area amplia en tierra (clase EsenciaVertida) y lluvia vertical aleatoria (clase CenizaSagrada).
...
*   **Fase 5 (Extincion - 10% a 0% HP):** Comportamiento frenetico que combina multiples clases de proyectiles simultaneamente e invocacion de esbirros de rango melee (MinionEspectral).

### Capa de Blindaje (HUD Dual)
El jefe cuenta con una barra de escudo que absorbe el 30% del dano total recibido. La renderizacion de esta capa en el canvas utiliza una composicion cromatica cyan en paralelo con la barra de salud real.

---

## 5. Arquitectura del Puente TikTok Live

La conexion de eventos de redes sociales con la logica del juego funciona de la siguiente manera:

1.  **Captura de Sockets:** El bridge se conecta a las APIs anonimas de TikTok simulando una sesion de espectador sobre el identificador de transmision provisto.
2.  **Mapeo de Regalos:** Un catalogo de referencias mapea los identificadores internos de regalos de TikTok a estructuras de datos in-game. Por ejemplo, el regalo de un cofre puede disparar una curacion al jefe en la partida de Travis, o un regalo de menor valor puede invocar proyectiles adicionales en pantalla.
3.  **Renderizacion Dinamica en Canvas:** Cuando el bridge emite un evento de regalo al juego, la clase liveOverlay captura el payload que contiene el nombre del remitente y la URL de la imagen del regalo. La imagen se descarga de forma asincrona desde el CDN de TikTok y se dibuja directamente en el canvas con efectos de desvanecimiento y transiciones fluidas de opacidad de forma dinamica, usando fallbacks graficos locales si la conexion al CDN falla.

---

## 6. Pipeline de Despliegue y Mantenimiento

Para facilitar el desarrollo seguro y despliegues rapidos sin interrumpir las pruebas activas en el VPS, se utiliza el siguiente fluxo de trabajo:

1.  **Desarrollo Local:** Ejecucion de Vite en entorno local con recarga en vivo de codigo. Las credenciales de integraciones de bases de datos y tokens se leen localmente desde archivos de entorno controlados y no commiteados (.env).
...
4.  **Despliegue Contenerizado:** El juego compilado se monta en los volumenes del contenedor de Caddy en produccion, mientras que los servidores Node.js (Bridge y TRAVIS) se reconstruyen dentro de la red aislada de Docker en el servidor para reflejar los ultimos cambios inmediatamente.
